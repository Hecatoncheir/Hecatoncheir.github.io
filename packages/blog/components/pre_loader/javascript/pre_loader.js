...
// Custom SETTINGS for each preLoader in related index.html
...
var preLoader = {
  ...
  initRebound: function initRebound() {
    ...
    var settings = preLoader.settings.rebound;

    // Create a SpringSystem.
    var springSystem = new rebound.SpringSystem();

    // Add a spring to the system.
    preLoader.spring = springSystem.createSpring(settings.tension, settings.friction);
  },
  ...
  initSpinner: function initSpinner() {
    ...
    var settings = preLoader.settings.spinner;

    // Instantiate Spinner.
    preLoader.spinner = new Spinner(settings);
  },
  ...
  init: function init() {
    ...
    preLoader.initRebound();
    preLoader.initSpinner();

    // Init animation with Rebound Spring System.
    preLoader.spinner.init(preLoader.spring, spinnerTypeAutoSpin);

    if (spinnerTypeAutoSpin) {
      // Fake loading time, in a real world just call preLoader.spinner.setComplete();
      // whenever the preload will be completed.
      // setTimeout(function () {
      //   preLoader.spinner.setComplete();
      // }, 6000);
    } else {
      // Perform real ajax request.
      preLoader.loadSomething();
    }
  },
  ...
  loadSomething: function loadSomething() {
    ...
    oReq.addEventListener('progress', function (oEvent) {
      if (oEvent.lengthComputable) {
        ...
        preLoader.spring.setEndValue(percent * 0.01);
      }
    });
...
document.addEventListener("Preloader must be visible", function(event) {
  console.log('event');
  preLoader.init();
});

document.addEventListener("Page ready", function(event) {
  preLoader.spinner.setComplete();
});